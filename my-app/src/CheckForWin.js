...
function CheckForHorizontalWin(board, column, row, ID) {
  if (
    board[column][row] === ID &&
    board[column + 1][row] === ID &&
    board[column + 2][row] === ID &&
    board[column + 3][row] === ID
  ) {
    return true;
  }
  return false;
...
function CheckForVerticalWin(board, column, row, ID) {
  if (
    board[column][row] === ID &&
    board[column][row + 1] === ID &&
    board[column][row + 2] === ID &&
    board[column][row + 3] === ID
  ) {
    return true;
  }
  return false;
...
function CheckForDiagonalWinLowerRighttoUpperLeft(board, column, row, ID) {
  if (
    board[column][row] === ID &&
    board[column - 1][row + 1] === ID &&
    board[column - 2][row + 2] === ID &&
    board[column - 3][row + 3] === ID
  ) {
    return true;
  }
  return false;
